...
def extract_specific_words(text, chemical_ingredients):
    ...
    for word in text.split():
        ...
        if not best_match.empty:
            ingredients.append(best_match.iloc[0]['Ingredient'])
    return ingredients
...
if __name__ == "__main__":
    ...
    # Display ingredients and their warnings
    if not ingredients:
        print("No ingredients extracted.")
    else:
        ingredient_warnings = chemical_ingredients[chemical_ingredients['Ingredient'].isin(ingredients)][['Ingredient', 'Warning']]
        print("Ingredient Warnings:")
        print(ingredient_warnings)
